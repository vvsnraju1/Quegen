...
def get_data_paths(input_file_name_1: str, input_file_name_2: Optional[str] = None,
    ...
    if len(sections_list) < 2:
        return {'message': 'sl:Section names could not be extracted. Please input the section names manually'}
...
def extract_toc_from_pdf(file_path):
    ...
    # Scan the first few pages to find a Table of Contents
    for page_number in range(5):  # Assume TOC is within the first 5 pages
        if page_number >= len(Document):
            break  # Prevent accessing a page that doesn't exist
        ...
        for line in lines:
            line = line.strip()
            # Check if line looks like a TOC entry (like "1. Introduction")
            if line and (line[0].isdigit() or "Chapter" in line or "Section" in line):
                toc_names.append(line)

    Document.close()
    return toc_names

def extract_toc_from_word(file_path):
    doc = Document(file_path)
    toc_names = []

    # Word TOC is typically within the first couple of paragraphs
    for para in doc.paragraphs[:15]:  # Check the first 15 paragraphs for TOC
        # Assuming TOC entries are formatted as Heading styles or follow a specific format.
        if para.style.name.startswith('Heading') or \
           any(keyword in para.text for keyword in ["Chapter", "Section", "Contents"]):
            toc_names.append(para.text.strip())
...
def get_section_names(file_type, file_path, file_name, temp_path, watermark):
    try:
        ...
        if not os.path.isfile(file_path):
            return {'message': 'The specified file does not exist.'}
        data = extract_text(file_type, file_path, file_name, temp_path, watermark)

        # You may want to call extract_text() if necessary, but in this case, we focus on TOC extraction
        if file_type == 'docx':
            sections_list = extract_toc_from_word(file_path)
        elif file_type == 'pdf':
            sections_list = extract_toc_from_pdf(file_path)
        else:
            return {'message': 'Unsupported file type'}
        
        # If no sections are found, return a message
        if not sections_list:
            return {'message': 'No section names found in the TOC.'}

        # Clean section names (optional: remove duplicates)
        sections_list = list(set(sections_list))  # Remove duplicates
        sections_list = [i.strip() for i in sections_list if i.strip()]

        return sections_list
    
    except ValueError as ve:
        print(f"ValueError: {ve}")
        return {'message': 've:Section names could not be extracted. Please input the section names manually.'}
    except AttributeError as ae:
        print(f"AttributeError: {ae}")
        return {'message': 'ae:Section names could not be extracted. Please input the section names manually.'}
    except TypeError as te:
        print(f"TypeError: {te}")
        return {'message': 'te:Section names could not be extracted. Please input the section names manually.'}
    except Exception as e:
        print(f"Exception: {e}")
        return {'message': 'e:Section names could not be extracted. Please input the section names manually.'}
